...
async fn inner_main() -> Result<(), String> {
    ...
    loop {
        ...
        if last.elapsed() >= Duration::from_secs(1) {
            ...
            let ttl_ticks = start.elapsed().as_secs_f64() * 32768.0f64;
            println!();
            println!("KiB/s: {:.02}", bytes as f64 / 1024.0);
            println!(
                "EXECUTOR:   IDLE: {:03.02}%, ACTV: {:03.02}%",
                100.0 * exec_ticks_idle as f64 / ttl_ticks,
                100.0 * exec_ticks_active as f64 / ttl_ticks,
            );
            bytes = 0;
            for (k, v) in vec {
                print!("T-{k:08X}: ");
